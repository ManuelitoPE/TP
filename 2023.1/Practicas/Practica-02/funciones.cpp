                      ...
                      double *ARRporcentajeAutores, const char *nomArch)
{
    ...
    while (true)
    {
        ...
        if (arch.eof())
            break;
        arch >> ARRcodigo_libroAutores[num_autores] >> ARRporcentajeAutores[num_autores];
        num_autores++;
    }
}
...
int buscarLibro(int codigoLibro, int *ARRcodigoLibro, int num_libro)
{
    ...
    {
        if (ARRcodigoLibro[i] == codigoLibro)
            break;
    }
    return i;
}
...
void encabezado(ofstream &arch, int fechaF, int fechaI)
{
    ...
    // 2023-12-22
    arch << setw(45) << "LIBRERIA LOS LIBROS_TP" << endl
         << right << endl
...
void sub_encabezado_2(ofstream &arch)
{
    linea(arch, MAX_LINEA, '-');
    arch << setw(5) << " "
         << "REGALIAS PARA LOS AUTORES" << endl;
    linea(arch, MAX_LINEA, '-');
